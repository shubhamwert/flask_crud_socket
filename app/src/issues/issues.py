...
from src.models.models import Issue,User,IssuePermission
from src.dbHandler.db import db
from src.schema.schema import IssueCreate, IssueRead,IssuePut
import json
...
issue_model = IssuesApiNs.model('Issue', {
    ...
    'status': fields.String(enum=['Open', 'triaged', 'in_progress', 'done'], default='Open'),
    'reported_by': fields.Integer(),
  
})
issue_put_model = IssuesApiNs.model('Issue', {
    'status': fields.String(enum=['Open', 'triaged', 'in_progress', 'done'],required=True, default='Open'),
  
})
issue_response_model = IssuesApiNs.inherit('IssueResponse', issue_model, {
    'id': fields.Integer()
})
...
class IssueList(Resource):
    ...
    @IssuesApiNs.doc(params={'X-User-ID': {'description': 'User ID', 'in': 'header', 'type': 'string', 'required': True}})
    def get(self):
        """List all issues"""
    ...
    def post(self):
        ...
        try:
            ...
            for users in permitted_users:
                socketio.emit('new_issue', IssueRead.model_validate(issue).model_dump(), room=f"user_{users.user_id}")
...
def has_edit_permissions(user_id, issue_id,):
    user = User.query.get(user_id)
    if not user:
        return False
